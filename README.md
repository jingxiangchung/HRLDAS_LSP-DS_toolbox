...
6. hrldas_deploy_ERA5.sh: a script to deploy HRLDAS for running our simulations forced by ERA5.
7. hrldas_deploy_regcm.sh: a script to deploy HRLDAS for running our simulations forced by RegCM. The run will not work with raw RegCM outputs, but require conversion using tools available here: https://github.com/RU-CORE-LAB/RegCM2HRLDAS/
8. extract_geo_em.sh: a script to extract the variable wanted stored in WRF geo_em file generated using geogrid.exe. CDO and NCO are required.
9. LULC_getinfo.sh: a script to extract the LULC information from WRF geo_em file's LU_INDEX to a .csv file. Required file 'cmLCZ_LULC_code.dat'. Please extract LU_INDEX from the geo_em file using "extract_geo_em.sh" first before running this script.
