...
#Preparations for HRLDAS simulation forced using RegCM's data
#This script helps to deploy HRLDAS LSP-DS by Ling Bo and Van Doan: https://github.com/xuelingbo/LSP-DS [Accessed on 23/9/2024] and RegCM2HRLDAS by Ratchanan Srisawadwong (Nick): https://github.com/RU-CORE-LAB/RegCM2HRLDAS [Accessed on 11/5/2026]
#Instruction: place this script inside the folder you are going to run HRLDAS

#user's input
hrldas_src='/home/jingxiang/climate_models/LSP-DS'
regcm2hrldas_src='/home/jingxiang/climate_models/RegCM2HRLDAS'
...
raw_dir='KL_EC-Earth3-Veg_historical'
...
cp ${regcm2hrldas_src}/HRLDAS_forcing_regcm/*.py .
...
#4. Preparing run_HRLDAS_regcm.py

echo "[hrldas_deploy] preparing run_HRLDAS_regcm.py file..."

#a. preparing months list & loop_start and loop_end information
...
sed -i "s/start_year = 2020/start_year = ${YYYY}/g;s/end_year = 2020/end_year = ${eYYYY}/g;s/'01','02','03','04','05','06','07','08','09','10','11','12'/${mm_list}/g;s/loop_start_date = '01-01'/loop_start_date = '${loop_start}'/g;s/loop_end_date = '12-31'/loop_end_date = '${loop_end}'/g" run_HRLDAS_regcm.py

#b. area
echo "...adding in domain information..."
sed -i "s/14.25, 99.9, 13.45, 101/${domain_area}/g" run_HRLDAS_regcm.py

#c. directories
echo "...adding in directories information..."
sed -i "s|= '/home/admin/WRF/RUN/LSP-DS/Data_114/run_regcm/RegCM_Data/CMCC-ESM2/ssp245/'|= '`pwd`/${raw_dir}/'|g;s|= '/home/admin/WRF/RUN/LSP-DS/Data_114/run_regcm/geo_em.d01.nc'|= '`pwd`/${geo_file}'|g;s|= '/home/admin/WRF/RUN/LSP-DS/Data_114/run_regcm/hrldas_simulation/'|= '`pwd`/${hrldas_out_dir}'|g" run_HRLDAS_regcm.py
